...
**File:** `dataScripts/plinko/duel-plinko-sim-1771364316980.json`\
...
### E27 — Verification Code
**File:** `src/plinko/PlinkoResultsGenerator.ts` and `tests/plinko/PlinkoResultsGeneratorTests.ts`\
**Description:** Independent TypeScript verification suite using Node.js `crypto` module. The results generator loads the dataset, maps seed hashes to revealed seeds, and recomputes every bet's final_slot using HMAC-SHA256 with hexToBytes key encoding. The test suite compares computed results against live API results.\
...
**Referenced in:** [Executive Summary](executive-summary.md), [Edge Case Analysis](edge-case-analysis.md), [Exploit Testing](exploit-testing.md)
...
### E50 — Exploit Category Assessment
...
### E56 — Expected vs Observed Streak Comparison
